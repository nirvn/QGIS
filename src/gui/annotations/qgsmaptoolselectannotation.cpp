...
{
  if ( QgsAnnotationLayer *annotationLayer = layer() )
  {
    QgsAnnotationItemEditContext context;
    context.setCurrentItemBounds( mMapCanvas->mapSettings().mapToLayerCoordinates( annotationLayer, mBoundingBox ) );
    ...
    QgsAnnotationItemEditOperationTranslateItem operation( mItemId, deltaX * mupp, deltaY * mupp, deltaX, deltaY );
...
void QgsMapToolSelectAnnotation::cadCanvasMoveEvent( QgsMapMouseEvent *event )
{
  ...
  {
    if ( !mDragging && mMouseHandles->sceneBoundingRect().contains( scenePos ) )
    {
...
void QgsMapToolSelectAnnotation::cadCanvasReleaseEvent( QgsMapMouseEvent *event )
{
  ...
  if ( !mDragging && !mSelectedItems.isEmpty() && mMouseHandles->sceneBoundingRect().contains( scenePos ) )
  {
