...
void QgsLayoutItemMap::recreateCachedImageInBackground()
{
  ...
  mCacheRenderingImage.reset( new QImage( std::floor( static_cast<double>( w ) * mPreviewDevicePixelRatio ),
                                          std::floor( static_cast<double>( h ) * mPreviewDevicePixelRatio ),
...
QgsMapSettings QgsLayoutItemMap::mapSettings( const QgsRectangle &extent, QSizeF size, double dpi, bool includeLayerSettings ) const
{
  ...
  if ( layout()->renderContext().isPreviewRender() )
  {
    ...
    jobMapSettings.setDevicePixelRatio( mPainter ? static_cast<float>( mPainter->device()->devicePixelRatioF() ) : 1.0 );
  }
