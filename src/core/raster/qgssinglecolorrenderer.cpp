...
QgsRasterBlock *QgsSingleColorRenderer::block( int bandNo, const QgsRectangle &extent, int width, int height, QgsRasterBlockFeedback *feedback )
{
  ...
  for ( qgssize i = 0; i < ( qgssize )width * height; i++ )
  {
    ...
    else if ( qgsDoubleNear( currentAlpha, 0.0 ) )
    {
      outputBlock->setColor( i, qRgba( 0, 0, 0, 0 ) );
    }
    else
    {
      outputBlock->setColor( i, qRgba( currentAlpha * mColor.red(), currentAlpha * mColor.green(), currentAlpha * mColor.blue(), currentAlpha * mColor.alpha() ) );
    }
  }
